...
function groupWines(records, priceField) {
  ...
  records.forEach(record => {
    ...
    // Pour Découverte, pas de vérification de prix
...
function generateCatalogPage(category, department, records, domainsMap) {
  ...
  // Si aucun vin pour cette catégorie/département, retourner null
...
// Main
console.log('📦 Génération des catalogues complets V4...\n');
...
['CHR', 'Caviste'].forEach(category => {
  DEPARTMENTS.forEach(dept => {
    ...
    if (html) {
      ...
      console.log(`⊘ ${filename} (aucun vin disponible)`);
    }
  });
});
